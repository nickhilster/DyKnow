...
- Persisting review decisions back into the proposal artifact: run `node packages/cli/dist/bin.js review --approve --page <page-id>`, `node packages/cli/dist/bin.js review --reject --all`, or `node packages/cli/dist/bin.js review --edit --page <page-id> --text <value>` after `dyknow update`. The current review slice updates proposal review states in place and can now persist one edited proposal text, but it does not yet regenerate drafts or launch an editor UI.
