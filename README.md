...
- Persisting review decisions back into the proposal artifact: run `node packages/cli/dist/bin.js review --approve --page <page-id>`, `node packages/cli/dist/bin.js review --reject --all`, `node packages/cli/dist/bin.js review --edit --page <page-id> --text <value>`, `node packages/cli/dist/bin.js review --skip --page <page-id>`, or `node packages/cli/dist/bin.js review --regenerate --page <page-id>` after `dyknow update`. The current review slice updates proposal review states in place, can persist one edited proposal text, can explicitly skip targeted proposals without mutating the snapshot, and can regenerate targeted proposals from the saved repo diff, but it does not yet launch an editor UI.
