...
- Inspecting recent review audit entries: run `node packages/cli/dist/bin.js log --limit 10` after `dyknow review`. The current implementation pretty-prints recent entries from `docs/dyknow/.state/audit-log.jsonl`, newest first, and remains read-only.
