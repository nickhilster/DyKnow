...
- Inspecting recent audit entries: run `node packages/cli/dist/bin.js log --limit 10` after `dyknow review`, `dyknow commit`, or `dyknow pr`. The current implementation pretty-prints recent entries from the committed `docs/dyknow/.state/audit-log.jsonl` artifact and the git-local runtime audit file used for confirmed external PR-open events, newest first, labels each entry with the source audit file it came from, and remains read-only.
