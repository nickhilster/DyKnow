...
- `dyknow review` now works for the first persisted edit, external-editor edit, skip, and regenerate slice, reading `docs/dyknow/.state/update-proposals.json`, applying approval, rejection, escalation, one edited proposal text change, one editor-driven edited proposal text change, an explicit skip action, or a targeted regenerate action while only mutating the artifact when the chosen action requires it.
