...
- `dyknow pr` now works for the first branch-and-PR slice, starting from the base branch, applying approved proposals into a fresh branch, carrying a `publish:pr-prepared` audit entry in the committed local flow before the external PR-open call, pushing it to `origin`, and opening a GitHub pull request with a page/risk/source summary table.
