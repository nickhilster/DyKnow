...
- `dyknow log` now works for the first read-only audit-viewer slice, reading `docs/dyknow/.state/audit-log.jsonl`, validating its JSONL entries, and pretty-printing the most recent review and publish audit entries first.
...
- `dyknow pr` now works for the first branch-and-PR slice, starting from the base branch, applying approved proposals into a fresh branch, carrying the corresponding publish audit entry in the committed flow, pushing it to `origin`, and opening a GitHub pull request with a page/risk/source summary table.
