...
- `dyknow log` now works for the first read-only audit-viewer slice, reading the committed `docs/dyknow/.state/audit-log.jsonl` artifact plus a git-local runtime audit file, validating those JSONL entries, and pretty-printing the most recent review and publish audit entries first.
