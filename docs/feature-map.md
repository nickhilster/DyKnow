...
| `dyknow pr` | Implemented (first slice) | Starting from the base branch, create a review branch, apply `Approved` proposals, carry a `publish:pr-prepared` audit entry in that committed local flow before the external PR-open call, push to `origin`, and open a GitHub pull request with a page/risk/source summary table. |
