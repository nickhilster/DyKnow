...
| `dyknow review` | Implemented (first slice) | Read `docs/dyknow/.state/update-proposals.json`, list proposal state counts, persist `Approved`, `Rejected`, or `Escalated` decisions, and replace one targeted proposal text while marking it `Edited`. Regenerate and skip flows remain planned. |
