...
| `dyknow log` | Implemented (first slice) | Read the committed `docs/dyknow/.state/audit-log.jsonl` artifact plus the git-local runtime audit file, validate each JSONL entry against the shared audit-entry schema, and pretty-print recent review and publish audit entries newest first without mutating the committed log artifact. |
