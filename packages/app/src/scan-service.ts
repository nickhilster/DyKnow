...
export type CreateRepoMapOptions = {
  configPath: string;
  cwd: string;
...
export type CreateRepoMapResult = {
  outputPath: string;
  repoMap: RepoMap;
};
...
export async function createRepoMapSnapshot(
  options: CreateRepoMapOptions,
): Promise<CreateRepoMapResult> {
  ...
  await writeFile(outputPath, `${JSON.stringify(repoMap, null, 2)}\n`, "utf8");
  ...
  if (blockingWarningMessage) {
    throw new Error(blockingWarningMessage);
  }
...
export { RepoMapWarningCodeSchema };
export { DEFAULT_REPO_MAP_OUTPUT_PATH };
