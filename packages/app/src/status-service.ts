...
const DEFAULT_AUDIT_LOG_PATH = "docs/dyknow/.state/audit-log.jsonl";
...
async function resolveRuntimeAuditPath(rootPath: string) {
  const gitDir = await runGit(rootPath, ["rev-parse", "--git-dir"]).catch(
    () => "",
  );
  ...
  return resolve(rootPath, gitDir, "dyknow-runtime-audit.jsonl");
}
