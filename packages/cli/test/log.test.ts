...
`,
  ...
  it("can filter the default log view down to committed entries only", async () => {
    const root = await mkdtemp(join(tmpdir(), "dyknow-log-"));
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    const exitCode = await runCli(["log", "--source", "committed"], {
      cwd: root,
      stdout: (message) => {
        stdout.push(message);
      },
    ...
    expect(stderr).toEqual([]);
    expect(stdout[0]).toContain("publish:pr-prepared by copilot");
    expect(stdout[0]).not.toContain("publish:pr-opened by copilot");
    expect(stdout[0]).toContain(
      "Recent audit entries from docs/dyknow/.state/audit-log.jsonl for source=committed (showing 1 of 1):",
    );
  ...
  it("shows only committed labels when source=all and only committed entries exist", async () => {
    const root = await mkdtemp(join(tmpdir(), "dyknow-log-"));
    const stdout: string[] = [];
    const stderr: string[] = [];
