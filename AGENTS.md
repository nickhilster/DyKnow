...
## Development commands
  ...
  - `node packages/cli/dist/bin.js log --limit 10 --source all` — pretty-print recent entries from `docs/dyknow/.state/audit-log.jsonl` and the git-local runtime audit file after a build, label each entry with its source audit file, and optionally filter to committed or runtime entries.
